...
const Pagination = ({dataPerPage, setCurrentPage, currentPage, toggleRecord}) => {
  ...
  const batchTestNumber = useSelector((state) => state.batch.testNumberArray);

  // For Search Technology
  const totalData = batch.searchBatch.length;
  console.log("total Data:",totalData);

  // For Record Data Technology
  const totalTestData = batchTestNumber.length;
  console.log("totalTestData",totalTestData);

  // For Search Technology
  let totalSearchData;
  if(totalData !== 0){
    totalSearchData = batch.searchBatch[0].length;
  ...
  if(totalTestData!==0){
    totalTestPageData = batchTestNumber[0].length;
