...
export const uploadQuestionByForm = createAsyncThunk(
  "/questions/upload-excel-questions",
  async ({ data,catId,techId }, { rejectWithValue}) => {
    try {
      const response = await axios.post(
        AXIOS_BASE_URL + `/questions/create-new?techId=${techId}&catId=${catId}`,
        data
      );
      return response.data;
    } catch (error) {
